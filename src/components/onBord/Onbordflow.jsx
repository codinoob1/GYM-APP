        ...
        <div className="rounded-[32px] border border-[#232630] bg-[#08090d] p-6 shadow-[0_40px_120px_rgba(0,0,0,0.35)]">
          <div className="flex flex-col gap-6 md:gap-0 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-4">
              <div className="h-12 w-12 rounded-2xl bg-[#c4f135] text-black grid place-items-center font-bold">G</div>
              <div>
                <p className="text-xs uppercase tracking-[0.35em] text-[#8b8d98]">Gym Tracker AI</p>
                <p className="text-sm text-[#8b8d98]">Step {activeStep + 1} of {steps.length}</p>
              </div>
            </div>
          ...
          <div className="relative mt-8 overflow-hidden rounded-full bg-[#11141f] h-2">
            <div className="absolute inset-y-0 left-0 rounded-full bg-[#c4f135] transition-all" style={{ width: `${((activeStep + 1) / steps.length) * 100}%` }} />
          </div>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-4">
            {steps.map((label, index) => {
              ...
              return (
                <div key={label} className="flex items-center gap-3 text-sm font-medium text-[#8b8d98]">
                  <div className={`flex h-9 w-9 items-center justify-center rounded-full border border-[#232630] ${isActive ? 'bg-[#c4f135] text-black' : isCompleted ? 'bg-[#1b2210] text-[#c4f135]' : 'bg-[#11141f] text-[#6e7387]'}`}>
                    {isCompleted ? '✓' : index + 1}
                  </div>
