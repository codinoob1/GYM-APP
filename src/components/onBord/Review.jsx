...
function formatDetails(exercise) {
  const weight = exercise.weight ? ` @ ${exercise.weight}${exercise.unit || 'kg'}` : '';
...
function LoadingSkeleton() {
  return (
    <div className="rounded-3xl border border-[#232630] bg-[#11141f] p-6 space-y-5">
      {[1, 2, 3].map((i) => (
        <div key={i} className="rounded-3xl border border-[#181b23] bg-[#0f131d] p-5 animate-pulse">
          ...
          <div className="mt-4 space-y-4">
            {[1, 2, 3].map((j) => (
              <div key={j} className="rounded-2xl border border-[#1f2430] bg-[#11141f] p-4">
        ...
        {!isLoading && !error && parsedPlan && (
          <div className="rounded-3xl border border-[#232630] bg-[#11141f] p-6">
            {parsedPlan.map((section) => (
              <div key={section.day} className="mb-5 last:mb-0 rounded-3xl border border-[#181b23] bg-[#0f131d] p-5">
                <div className="flex items-center justify-between gap-4 pb-4 border-b border-[#232630]">
                  <div className="flex items-center gap-3">
                    <span className={`inline-flex h-3.5 w-3.5 rounded-full ${badgeColor(section.category)}`} />
                ...
                <div className="mt-4 space-y-4">
                  {section.exercises.map((exercise) => (
                    <div key={exercise.name} className="rounded-2xl border border-[#1f2430] bg-[#11141f] p-4">
                      ...
                      <p className="text-sm text-[#8b8d98] mt-1">{formatDetails(exercise)}</p>
                    </div>
                  ))}
                </div>
              </div>
